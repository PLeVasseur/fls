...
def main() -> int:
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Build and compare rendered HTML output under configured policy",
        )
        parser.add_argument(
            "--mode",
        ...
        if args.mode == "refs":
            ...
            report_path = resolve_report_path(
                ...
                repo_root,
                report_root,
                f"verify-html-diff-refs-{sanitize_label(left_ref)}-vs-{sanitize_label(right_ref)}.txt",
            )
...
def run_comparison_mode(
    ...
    if has_differences(diff):
        print(f"differences found under configured comparison policy; report written to {report_path}")
        return EXIT_DIFFERENCES
    print(f"no differences under configured comparison policy; report written to {report_path}")
    return EXIT_SUCCESS
...
def export_ref(
    ...
    if worktree_dir is None:
        worktree_dir = Path(tempfile.mkdtemp(prefix="worktree-", dir=str(worktree_root)))
    ...
    try:
        ...
        run(["git", "worktree", "add", "--detach", str(worktree_dir), ref], cwd=repo_root)
        worktree_added = True
        run(["./make.py", "--clear"], cwd=worktree_dir)
