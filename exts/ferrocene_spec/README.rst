...
The extension emits both ``paragraph-ids.json`` and ``paragraph-ids-rich.json``
in the build output directory. The rich artifact is schema-backed by
``exts/ferrocene_spec/schemas/paragraph-ids-rich.schema.json``.
