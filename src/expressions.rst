...
.. syntax::
   ...
   MatchArmGuardChain ::=
       MatchArmGuardConditionOperandBeforeLet* MatchArmGuardLetPattern MatchArmGuardConditionAfterLet*

   MatchArmGuardConditionOperandBeforeLet ::=
       MatchArmGuardConditionOperand $$&&$$

   MatchArmGuardConditionAfterLet ::=
   ...
   MatchArmGuardScrutinee ::=
       Expression

:dp:`fls_3TGpt9OGydQi`
A :s:`MatchArmGuardConditionOperand` is any :t:`expression` in category
:s:`Expression`, except expressions in any of the following categories:
...
:dp:`fls_7OkU5RVrAyev`
A :s:`MatchArmGuardScrutinee` is any :t:`expression` in category
:s:`Expression`, except expressions in any of the following categories:
...
:t:`[match arm guard condition]s`, separated by ``&&``, that contains at least
one :t:`match arm guard let pattern`.

:dp:`fls_gNQBzI92IAir`
A :t:`match arm guard let pattern` in a :t:`match arm guard chain` is what
distinguishes the chain from a :t:`match arm guard operand`; a match arm guard
that has no :t:`match arm guard let pattern` has a
:t:`match arm guard operand`.

:dp:`fls_Uv7QsP2nKcDx`
A :t:`match arm guard operand` is the :t:`expression` in category
:s:`MatchArmGuardOperand` of a :t:`match arm guard` that does not have a
:t:`match arm guard chain`.

:dp:`fls_Er5TmL9bXwVq`
A :t:`match arm guard condition operand` is the :t:`expression` in category
:s:`MatchArmGuardConditionOperand` of a :t:`match arm guard condition`.

:dp:`fls_Yq4NhC8sRpJm`
A :t:`match arm guard scrutinee` is the :t:`expression` in category
:s:`MatchArmGuardScrutinee` of a :t:`match arm guard let pattern`.
